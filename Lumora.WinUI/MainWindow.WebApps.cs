...
// Applications web Lumora : un site épinglé, ouvert dans une fenêtre dédiée
// (LumoraAppWindow) sans onglets ni barre d'adresse, comme le mode « application »
...
public sealed partial class MainWindow
{
    ...
    private void RefreshWebAppsPanel()
    {
        ...
        if (apps.Count == 0)
        {
            ...
            {
                Text = "Aucune application installée. Utilisez « Installer la page active » ci-dessus, ou depuis le menu Lumora.",
                Opacity = 0.65,
    ...
    private UIElement BuildWebAppElement(LumoraWebApp app)
    {
    ...
    private void LaunchWebAppWindow(LumoraWebApp app)
    {
        var appWindow = new LumoraAppWindow(app, _profile);
        appWindow.Activate();
        appWindow.InitializeBrowserSurface();
    }

    private async Task RenameWebAppAsync(LumoraWebApp app)
    {
    ...
    private async Task DeleteWebAppAsync(LumoraWebApp app)
    {
    ...
    private async void InstallAppMenu_Click(object sender, RoutedEventArgs e)
    {
        ...
        var info = new TextBlock
        {
            ...
            Opacity = 0.65,
            FontSize = 12,
            Text = $"Ouvre {rootDomain} dans une fenêtre dédiée, sans onglets ni barre d'adresse. La session de ce site sera automatiquement conservée après fermeture de Lumora. Les protections (bloqueur de pubs/trackers, anti-télémétrie, HTTPS) restent actives."
        };
        ...
        var app = existing ?? new LumoraWebApp();
        app.Title = title;
        app.Url = tab.Address;
        ...
        // vouloir y rester connecté après fermeture de Lumora.
    ...
    private void InstallShortcutsForApp(LumoraWebApp app, bool wantDesktop)
    {
        if (!string.IsNullOrWhiteSpace(app.ShortcutFileName))
        {
            ShellShortcut.Delete(Path.Combine(ShellShortcut.StartMenuAppsFolder(), app.ShortcutFileName));
    ...
    private string? ResolveShortcutIconPath(LumoraWebApp app)
    {
        if (!string.IsNullOrWhiteSpace(app.IconFile))
        {
