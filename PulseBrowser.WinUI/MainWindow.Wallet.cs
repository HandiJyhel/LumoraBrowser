...
// Portefeuille : moyens de paiement stockés UNIQUEMENT dans vault.pulse.
...
public sealed partial class MainWindow
{
    ...
    private async Task RegisterPaymentMonitorAsync(CoreWebView2 core)
    {
        try
        {
            await core.AddScriptToExecuteOnDocumentCreatedAsync("""
                (function(){
                    if(window.__pulse_payment_monitor)return;
                    window.__pulse_payment_monitor=true;
                    var reported=false;
                    ...
                    function detect(){
                        ...
                        try{window.chrome.webview.postMessage(JSON.stringify({t:'pulse.payment.form',o:location.origin}));}catch(_){}
                    }
                    function start(){
                        detect();
                        if(!reported){
    ...
    private async Task<(bool Success, string Message)> FillCardAsync(VaultPaymentCard card)
    {
        ...
        try
        {
            var script = PaymentFillScript.Replace("__PULSE_CARD_PAYLOAD__", payload);
    ...
    private const string PaymentFillScript = """
        (function(){
            var card=__PULSE_CARD_PAYLOAD__;
            function fire(el){
